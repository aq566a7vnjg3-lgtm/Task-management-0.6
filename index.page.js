import {
  ...
  function applySort(list){
    ...
    const hopeISO =
      (activeCase.derived && activeCase.derived.hopeNoApplyDueDate) ? activeCase.derived.hopeNoApplyDueDate :
      ((activeCase.tasks||[]).find(t=>t.key==='hope_no_apply')?.dueDate || null);
    ...
    tops.forEach(x=>{
      ...
      // 登録日の場合は、右側に希望番号申請期限を表示
      const mainDate = `${fmtJP(x.iso)}(${weekdayJP(x.iso)})`;
      const dateHtml = (x.label==='登録日' && hopeISO)
        ? `<div class="d dline"><span class="mainDate">${mainDate}</span><span class="inlineDue hopeDue">希望番号申請：${fmtJP(hopeISO)}(${weekdayJP(hopeISO)})</span></div>`
        : `<div class="d">${mainDate}</div>`;
      card.innerHTML = `<div class="t">${x.label}</div>${dateHtml}<div class="p">${cls==='overdue'?'期限切れ':cls==='soon'?'まもなく':' '}</div>`;
